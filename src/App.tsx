...
type AICourse = { title: string; summary: string; weeklyMinutes: number; modules: { title: string; description: string; lessons: { title: string; skill: string; minutes: number }[] }[]; recommendations: string[] }
...
function Overview({ lessons, activeModule, setActiveModule, beginLesson, activeLesson, answer, setAnswer, finishLesson, user }: { lessons: Lesson[]; activeModule: string; setActiveModule: (value: string) => void; beginLesson: (lesson: Lesson) => void; activeLesson: Lesson | null; answer: string | null; setAnswer: (answer: string) => void; finishLesson: () => void; user: User }) {
  ...
  return <>
    ...
    {activeLesson ? <Practice lesson={activeLesson} answer={answer} setAnswer={setAnswer} finishLesson={finishLesson} /> : <section className="focus-card reveal"><div className="focus-copy"><div className="section-kicker"><span className="live-dot" /> PASUL RECOMANDAT PENTRU TINE</div><h2>{featured.title}</h2><p>Începe cu o lecție scurtă și exersează engleza de care ai nevoie la muncă.</p><div className="focus-meta"><span><Clock3 size={13} /> {featured.duration}</span><span><MessageCircle size={13} /> {featured.meta}</span><span><Award size={13} /> Nivel {featured.level}</span></div><button className="primary-button" onClick={() => beginLesson(featured)}>Începe lecția <ArrowRight size={15} /></button></div><div className="focus-visual"><div className="sun-shape" /><div className="visual-label">Pasul 1 din 3</div><span className="visual-line line-one" /><span className="visual-line line-two" /></div></section>}
    <section className="quick-start reveal"><div className="quick-start-heading"><div><p className="eyebrow">CUM FUNCȚIONEAZĂ</p><h2>Începi simplu, progresezi constant</h2></div><span>3 pași până la următoarea abilitate</span></div><div className="quick-start-steps"><div><strong>01</strong><h3>Alege un path</h3><p>Selectează domeniul profesional pe care vrei să îl exersezi.</p></div><div><strong>02</strong><h3>Fă o lecție scurtă</h3><p>Învață prin situații reale, cu feedback imediat.</p></div><div><strong>03</strong><h3>Urmărește progresul</h3><p>Revino când ai timp. Parcursul tău se salvează automat.</p></div></div></section>
...
function Paths({ paths, onAdd }: { paths: LearningPath[]; onAdd: () => void }) {
  const [course, setCourse] = useState<AICourse | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const generateCourse = async () => {
    const profile = JSON.parse(localStorage.getItem('linguapro-profile') ?? '{}') as { language?: string; domains?: string[]; level?: string; goal?: string }
    setLoading(true)
    setError('')
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:3001'}/api/v1/ai/learning-path`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ language: profile.language ?? paths[0]?.language ?? 'Engleză', domains: profile.domains ?? paths.map((path) => path.domain), level: profile.level ?? paths[0]?.level ?? 'B1', goal: profile.goal ?? 'Să comunic mai sigur la muncă.' }) })
      const data = await response.json()
      if (!response.ok) throw new Error(data.detail ?? data.error ?? 'Nu am putut genera cursul.')
      setCourse(data.course)
    } catch (generationError) { setError(generationError instanceof Error ? generationError.message : 'AI-ul nu este disponibil momentan.') } finally { setLoading(false) }
  }
  return <Page title="Path-urile mele" eyebrow="PARCURSURI PERSONALIZATE" description="Învață mai multe limbi sau aplică aceeași limbă în domenii profesionale diferite."><div className="path-toolbar"><div><strong>{paths.length || 0} path-uri active</strong><small>Fiecare are propriul progres și obiectiv.</small></div><div className="path-actions"><button className="secondary-button" onClick={generateCourse} disabled={loading}>{loading ? 'Generează...' : 'Generează curs cu AI'} <Sparkles size={15} /></button><button className="primary-button" onClick={onAdd}>Adaugă path <ArrowRight size={15} /></button></div></div>{error && <div className="ai-error">{error}</div>}{course && <AICourseCard course={course} />}{paths.length ? <div className="paths-grid">{paths.map((path) => <article className={path.active ? 'path-card active' : 'path-card'} key={path.id}><div className="path-card-top"><span className="path-language">{path.language}</span><span className="path-status">{path.active ? 'Activ' : 'În așteptare'}</span></div><h2>{path.language} pentru {path.domain}</h2><p>Nivel {path.level} · {path.lessons} lecții profesionale</p><div className="path-progress"><div><span>Progres</span><strong>{path.progress}%</strong></div><div className="bar"><i style={{ width: `${path.progress}%` }} /></div></div><button className="path-link">Deschide path <ArrowRight size={14} /></button></article>)}</div> : <div className="empty-paths"><Layers3 size={26} /><h2>Construiește primul tău path</h2><p>Alege limba și domeniile care contează pentru munca ta.</p><button className="primary-button" onClick={onAdd}>Începe configurarea <ArrowRight size={15} /></button></div>}</Page>
}

function AICourseCard({ course }: { course: AICourse }) { return <section className="ai-course-card"><div className="ai-course-header"><div><p className="eyebrow"><Sparkles size={13} /> GENERAT DE AI</p><h2>{course.title}</h2><p>{course.summary}</p></div><span>{course.weeklyMinutes} min / săptămână</span></div><div className="ai-modules">{course.modules.map((module) => <article key={module.title}><strong>{module.title}</strong><p>{module.description}</p><ul>{module.lessons.map((lesson) => <li key={lesson.title}>{lesson.title}<small>{lesson.skill} · {lesson.minutes} min</small></li>)}</ul></article>)}</div><div className="ai-recommendations"><strong>Recomandări pentru tine</strong>{course.recommendations.map((recommendation) => <span key={recommendation}>{recommendation}</span>)}</div></section> }
...
function Footer({ onNavigate }: { onNavigate: (view: View) => void }) { return <footer className="site-footer"><div className="footer-main"><div className="footer-brand"><div className="brand"><span className="brand-mark">A</span><span>altera</span></div><p>Învățare profesională, construită în jurul muncii tale.</p><span className="footer-status"><i /> Platformă operațională</span></div><div className="footer-column"><strong>Învață</strong><button onClick={() => onNavigate('paths')}>Path-urile mele <ArrowUpRight size={13} /></button><button onClick={() => onNavigate('library')}>Biblioteca <ArrowUpRight size={13} /></button><button onClick={() => onNavigate('progress')}>Progres <ArrowUpRight size={13} /></button></div><div className="footer-column"><strong>Practică</strong><button onClick={() => onNavigate('plan')}>Planul meu <ArrowUpRight size={13} /></button><button onClick={() => onNavigate('recommendations')}>Recomandări <ArrowUpRight size={13} /></button><button onClick={() => onNavigate('speaking')}>Practică orală <ArrowUpRight size={13} /></button></div><div className="footer-column"><strong>Ajutor</strong><button onClick={() => onNavigate('settings')}>Setări cont <ArrowUpRight size={13} /></button><button onClick={() => onNavigate('assessment')}>Refă profilul <ArrowUpRight size={13} /></button><span>support@altera.demo</span></div></div><div className="footer-bottom"><span>© 2024 Altera Learning Platform</span><span>Creat pentru învățare aplicată în contexte profesionale</span><span>Privacy · Terms</span></div></footer> }
...
function Page({ title, eyebrow, description, children }: { title: string; eyebrow: string; description: string; children: React.ReactNode }) { return <section className="page-view reveal"><header className="page-header"><div><p className="eyebrow">{eyebrow}</p><h1 className="page-title">{title}</h1><p className="page-description">{description}</p></div><div className="page-header-meta"><span className="page-meta-label">SPAȚIU DE LUCRU</span><span className="page-meta-value">Actualizat acum</span></div></header><div className="page-content">{children}</div></section> }
